...
class ParkingCubit extends Cubit<ParkingStates> {
  ...
  void updateUserData({@required String username, @required String email, @required String phone, @required String carStr, @required String carInt,})
  {emit(ParkingLoadingUpdateState());
    ...
    DioHelper.putData(
      ...
      token: token,
      data: {
        'username' : username, 'email' : email, 'phone' : phone, 'carStr' :carStr, 'carInt' :carInt,
      },
    ).then((value) {
      userModel = ParkingLoginModel.fromJson(value.data);
      printFullText(userModel.data.username);
